...
from keras.layers import Dense, Dropout, Conv2D, LayerNormalization
...
"""
Full Credit to
@Author: yingkaisha, https://github.com/yingkaisha
...
def window_partition(x, window_size):
    ...
    # Reshape the input tensor to patches
...
class patch_extract(Layer):
    ...
    def get_config(self):
        return {'patch_size': (self.patch_size_x, self.patch_size_y)}
...
class patch_embedding(Layer):
    ...
    def get_config(self):
        return {'num_patch': self.num_patch, 'embed_dim': self.proj.units}
...
class patch_merging(tf.keras.layers.Layer):
    ...
    def get_config(self):
        return {'num_patch': self.num_patch, 'embed_dim': self.embed_dim}
...
class patch_expanding(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        # rearrange depth to number of patches
    ...
    def get_config(self):
        return {'num_patch': self.num_patch, 'embed_dim': self.embed_dim, 'upsample_rate': self.upsample_rate,
...
class SwinTransformerBlock(tf.keras.layers.Layer):
    ...
    def get_config(self):
        return {'dim': self.dim, 'num_patch': self.num_patch, 'num_heads': self.num_heads,
                'window_size': self.window_size, 'shift_size': self.shift_size, 'num_mlp': self.num_mlp}
